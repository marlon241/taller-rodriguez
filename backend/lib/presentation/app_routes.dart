...
class AppRoutes {
  ...
  final ReportesController _reportesController;
  ...
  AppRoutes() 
      : _facturacionController = getIt<FacturacionController>(),
        ...
        _reportesController = getIt<ReportesController>(),
  ...
  Router get router {
    ...
    router.get('/api/ofertas/<id>', _obtenerOfertaPorId);
    ...
    router.get('/api/reportes/kpis', _obtenerKpis);
    router.get('/api/reportes/ventas', _obtenerVentas);
    router.get('/api/reportes/categorias', _obtenerCategorias);
    
    router.get('/api/facturas', _obtenerFacturas);
    router.get('/api/facturas/<id>', _obtenerFacturaPorId);
  ...
  Future<Response> _obtenerOfertaPorId(Request request, String id) async {
    final idInt = int.tryParse(id);
    if (idInt == null) {
      return Response.badRequest(body: '{"success": false, "message": "ID inválido"}', headers: _jsonHeaders);
    }
    final resultado = await _ofertaController.obtenerOfertaPorId(idInt);
    return Response.ok(resultado, headers: _jsonHeaders);
  }
  ...
  Future<Response> _crearOferta(Request request) async {
    try {
      ...
      final resultado = await _ofertaController.crearOferta(Map<String, dynamic>.from(data));
      return Response.ok(resultado, headers: _jsonHeaders);
    } catch (e) {
      return Response.badRequest(body: '{"success": false, "message": "Error al parsear el cuerpo de la solicitud"}', headers: _jsonHeaders);
    }
  }
  ...
  Future<Response> _actualizarOferta(Request request, String id) async {
    try {
      ...
      final body = await request.readAsString();
      final data = json.decode(body);
      final resultado = await _ofertaController.actualizarOferta(idInt, Map<String, dynamic>.from(data));
      return Response.ok(resultado, headers: _jsonHeaders);
    } catch (e) {
      return Response.badRequest(body: '{"success": false, "message": "Error al parsear el cuerpo de la solicitud"}', headers: _jsonHeaders);
    }
  ...
  Future<Response> _obtenerFacturaPorId(Request request, String id) async {
    final idInt = int.tryParse(id);
    if (idInt == null) {
      return Response.badRequest(
        body: json.encode({'success': false, 'message': 'ID inválido'}),
    ...
    final resultado = await _facturacionController.obtenerFacturaPorId(idInt);
    return Response.ok(resultado, headers: _jsonHeaders);
  }
  ...
  Future<Response> _obtenerKpis(Request request) async {
    final periodo = request.url.queryParameters['periodo'] ?? 'mes';
    final resultado = await _reportesController.obtenerKpis(periodo: periodo);
    return Response.ok(resultado, headers: _jsonHeaders);
  }
  
  Future<Response> _obtenerVentas(Request request) async {
    final periodo = request.url.queryParameters['periodo'] ?? 'mes';
    final resultado = await _reportesController.obtenerVentasPorSemana(periodo: periodo);
    return Response.ok(resultado, headers: _jsonHeaders);
  }
  
  Future<Response> _obtenerCategorias(Request request) async {
    final resultado = await _reportesController.obtenerVentasPorCategoria();
    return Response.ok(resultado, headers: _jsonHeaders);
  }
