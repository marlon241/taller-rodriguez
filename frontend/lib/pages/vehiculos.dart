...
import 'package:frontend/widgets/modals/visualizar_vehiculo_modal.dart';
import 'package:frontend/widgets/modals/editar_vehiculo_modal.dart';
...
class _VehiculosPageState extends State<VehiculosPage> {
  ...
  Future<void> _cargarVehiculos() async {
    ...
    final result = await VehiculoService.obtenerVehiculos(entregados: _mostrarEntregados);
    setState(() {
      _cargando = false;
      if (result['success'] == true) {
        _vehiculos = List<Map<String, dynamic>>.from(result['data']);
      }
    });
  }
  ...
  Future<void> _eliminarVehiculo(int id) async {
    ...
    if (confirm == true) {
      final result = await VehiculoService.eliminarVehiculo(id);
      if (result['success'] == true) {
        _cargarVehiculos();
        if (mounted) {
  ...
  void _visualizarVehiculo(Map<String, dynamic> v) {
    showDialog(
      context: context,
      builder: (_) => VisualizarVehiculoModal(vehiculo: v),
    );
  }

  void _editarVehiculo(Map<String, dynamic> v) {
    showDialog(
      context: context,
      builder: (_) => EditarVehiculoModal(
        vehiculo: v,
        onVehiculoEditado: _cargarVehiculos,
      ),
    );
  }
  ...
  List<Map<String, dynamic>> get _vehiculosFiltrados {
    ...
    return _vehiculos.where((v) {
      final matchSearch = query.isEmpty ||
          (v['placa'] ?? '').toString().toLowerCase().contains(query) ||
          (v['modelo'] ?? '').toString().toLowerCase().contains(query) ||
          (v['marca'] ?? '').toString().toLowerCase().contains(query) ||
          v['id'].toString().contains(query);

      final matchEstado = _filtroEstado == null || v['estado'] == _filtroEstado;
              ...
              child: Column(
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: isWide
                        ? Row(children: [
                            Expanded(child: SearchField(hint: 'Buscar vehiculo', controller: _searchController, onChanged: (val) => setState(() {}))),
                        ...
                        : Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                            SearchField(hint: 'Buscar vehiculo', controller: _searchController, onChanged: (val) => setState(() {})),
                  ...
                  _cargando
                      ...
                      : isWide
                          ? Container(
                              ...
                              child: _vehiculosFiltrados.isEmpty ? _buildEmptyState() : _buildTable(),
                            )
                          : _vehiculosFiltrados.isEmpty ? _buildEmptyStateMobile() : _buildCardList(),
                  const SizedBox(height: 20),
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 25),
                    child: isWide
                        ? Center(child: _AgregarVehiculoButton(isWide: isWide, onVehiculoAgregado: _cargarVehiculos))
                        : _AgregarVehiculoButton(isWide: isWide, onVehiculoAgregado: _cargarVehiculos),
                  ),
                ],
              ),
            ),
  ...
  Widget _buildToggle() {
    return Row(
      ...
      children: [
        const Text('Cambiar a entregados', style: TextStyle(fontSize: 13, color: Colors.black54)),
        const SizedBox(width: 8),
        Switch(
          value: _mostrarEntregados,
          activeColor: const Color(0xFFC0392B),
          onChanged: (val) {
            setState(() { _mostrarEntregados = val; _filtroEstado = null; });
            _cargarVehiculos();
          },
        ),
      ],
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 16),
                Text('NO SE HA AGREGADO NINGÚN VEHÍCULO', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold, letterSpacing: 1)),
              ],
            ),
          ),
        ],
  ...
  Widget _buildTableHeader() {
    const style = TextStyle(color: _headerText, fontWeight: FontWeight.bold, fontSize: 13);
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
        ...
        return Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              ...
              decoration: const BoxDecoration(color: _headerBg, borderRadius: BorderRadius.vertical(top: Radius.circular(12))),
              child: Row(children: [
                Text('#${v['id']}', style: const TextStyle(color: _headerText, fontWeight: FontWeight.bold, fontSize: 13)),
                const SizedBox(width: 8),
                Text(v['placa'] ?? '', style: const TextStyle(color: _headerText, fontWeight: FontWeight.bold, fontSize: 15)),
                const SizedBox(width: 8),
                Expanded(child: Text('${v['marca']} ${v['modelo']}', style: const TextStyle(color: _headerText, fontSize: 13))),
            ...
            Padding(
              ...
              child: Column(children: [
                _cardRow('Modelo', '${v['marca']} ${v['modelo']}'),
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                ...
                const SizedBox(height: 10),
                Row(children: [
                  Expanded(child: _AccionButton(label: 'Visualizar', color: const Color(0xFF2979FF), onTap: () => _visualizarVehiculo(v), fullWidth: true)),
                  const SizedBox(width: 6),
                  Expanded(child: _AccionButton(label: 'Editar', color: const Color(0xFFC0392B), onTap: () => _editarVehiculo(v), fullWidth: true)),
                  const SizedBox(width: 6),
                  Expanded(child: _AccionButton(label: 'Eliminar', color: const Color(0xFF7B1111), onTap: () => _eliminarVehiculo(v['id']), fullWidth: true)),
                ]),
              ]),
            ),
          ]),
  ...
  Widget _cardRow(String label, String value) {
    return Row(children: [
      SizedBox(width: 80, child: Text(label, style: const TextStyle(fontSize: 13, color: Colors.grey, fontWeight: FontWeight.w500))),
...
class _EstadoChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(estado, textAlign: TextAlign.center, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: text)),
    );
  }
}
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          child: Text(widget.label, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold, fontFamily: 'Itim')),
        ),
      ),
    );
  }
...
class _AgregarVehiculoButton extends StatefulWidget {
  ...
  const _AgregarVehiculoButton({required this.isWide, required this.onVehiculoAgregado});
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        onTap: () async {
          await showDialog(
            context: context,
            builder: (context) => AgregarVehiculoModal(onVehiculoAgregado: widget.onVehiculoAgregado),
          );
        },
        child: AnimatedContainer(
          duration: const Duration(milliseconds: 150),
          ...
          child: const Text('Agregar vehiculo', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: 16, fontFamily: 'Itim')),
        ),
      ),
    );
  }
